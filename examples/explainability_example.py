...
def show_shap_summary(dataset, labels):
    ...
    if sparse.issparse(embedding):
        df = pd.DataFrame.sparse.from_spmatrix(embedding, columns=["node "+str(node) for node in node_indexes])
    else:
        df = pd.DataFrame(data=embedding, columns=["node "+str(node) for node in node_indexes])
    ...
    for i in range(labels.shape[1]):
        shap_plot = shap.summary_plot(shap_values[i], df, show=False, plot_size=None)
